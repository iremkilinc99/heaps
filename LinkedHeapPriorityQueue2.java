public class LinkedHeapPriorityQueue2<K, V> implements PriorityQueue<K,V>{
    ...
    private class Node<K, V> implements Position<K, V> {
        ...
        public Node(K k, V v, Node<K, V> above, Node<K, V> leftChild, Node<K, V> rightChild, Node<K, V> pred) {
            key = k;
            value = v;
            parent = above;
            left = leftChild;
    ...
    protected Node<K, V> createNode(K k, V v, Node<K, V> parent, Node<K, V> left, Node<K, V> pred, Node<K, V> succ) {
        return new Node<K, V>(k, v, parent, left, pred, succ);
    }
    ...
    public Position<K, V> addRoot(K k, V v) throws IllegalStateException {
        ...
        root = createNode(k, v, null, null, null, null);
        size = 1;
        depth++;
        lastNode = root;
    ...
    public Position<K, V> addLeft(Position<K, V> p, K k, V v) throws IllegalArgumentException {
        ...
        Node<K, V> child = createNode(k, v, node, null, null, null);
        node.setLeft(child);
        size++;
        ...
        if (isEmpty()) {
            ...
            if (lastNode != root && lastNode.getParent().getLeft() == lastNode) {// eger yeni eklenecek node son eklenin sag kardesi ise
                ...
                if (size == (Math.pow(2, depth) - 1)) {// eger yeni levela geciyorsak
                    ...
                    Node<K, V> node = lastNode;
                    while (temp == null) {
                        if (sibling(node.getParent()) == null){// eger son eklenmis node sagda ise yukari cikip, yeni parent nodu bulup elimizdeki nodeu onun soluna ekleyecegiz
                            node = node.getParent();
                        }else{
